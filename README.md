...
| Package                                                                 | What it is                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`@effect-uai/core`](./packages/core)                                   | The primitives: `Loop`, `LanguageModel`, `Tool`, `Toolkit`, `Items`, `Turn`, `Transcriber`, `SpeechSynthesizer`, `RealtimeSession`, `EmbeddingModel`, `Reranker`, `DecisionModel`, `Chunker`, `Tokenizer`, `MusicGenerator`, `ImageGenerator`, `WebSearch`, `WebRead`, `Browser`, `Sandbox`, `DeepResearch`, `Messenger`. No provider deps. |
...
| [`@effect-uai/google`](./packages/providers/google)                     | Google Gemini: language model, embeddings, speech (sync STT + TTS), Gemini Live realtime sessions (voice and camera), Nano Banana image generation, and Lyria music generation.                                                                                                                                                             |
...
| [`@effect-uai/openai`](./packages/providers/openai)                     | OpenAI brand package: Responses language models, embeddings, deep research, image generation (`gpt-image-2`, with partial-image streaming), speech (`Transcriber` sync + realtime WS, `Synthesizer` sync + chunked HTTP), and OpenAI Realtime speech-to-speech sessions.                                                                    |
...
| [`@effect-uai/typesafe-ai`](./packages/providers/typesafe-ai)           | TypeSafe AI Jev `DecisionModel`: typed classify, rate and probability questions about one input, answered with probabilities in one call.                                                                                                                                                                                                   |
...
| [`@effect-uai/telegram`](./packages/providers/telegram)                 | Telegram `Messenger`: your agent as a bot over the Bot API. DMs, group mentions and commands in, streamed HTML replies, media and reactions out. Long-polling, no SDK.                                                                                                                                                                      |
| [`@effect-uai/discord`](./packages/providers/discord)                   | Discord `Messenger`: one gateway websocket plus the v10 REST API. DMs, mentions, threads and buttons in, streamed markdown replies, media and reactions out. No discord.js.                                                                                                                                                                 |
| [`@effect-uai/slack`](./packages/providers/slack)                       | Slack `Messenger`: Socket Mode plus the Web API. Mentions, DMs, slash commands and buttons in, streamed markdown replies in a thread, files and reactions out. No Bolt.                                                                                                                                                                     |
...
│       ├── google/            # @effect-uai/google - Gemini + speech + Live + images + Lyria
│       ├── mistral/           # @effect-uai/mistral - LLM + Voxtral speech (STT/TTS)
│       ├── openai/            # @effect-uai/openai - Responses + embeddings + speech + realtime + images
...
│       ├── typesafe-ai/       # @effect-uai/typesafe-ai - Jev decision model
...
├── recipes/                   # 42 worked recipes (type-checked, tested) covering
│                              # tools, approvals, fallback, voice, realtime, retrieval, MCP, chat bots, …
