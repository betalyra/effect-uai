...
- **`LanguageModel.retry(schedule)`** — stream combinator that retries
  only the retryable subset of `AiError` (`RateLimited`, `Unavailable`,
  `Timeout`). Other failures propagate unchanged.
