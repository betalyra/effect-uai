...
See the `sandbox-code-interpreter` recipe and the
...
- **`sandbox-code-interpreter`** (in `recipes-extras/`): the
  "run, fix, repeat" pattern. The model writes Python, the sandbox
  runs it, stderr feeds back into the next turn until the code works.
  Lives in `recipes-extras/` because it needs a sandbox provider
  running.
...
Point Claude Code (or Cursor, …) at this page. The rewrites are
mechanical enough that any tool-using LLM can apply them across your
codebase.
