...
description: A tool-layer refactor. A Toolkit is now a name-indexed record built with Toolkit.make, plain and streaming tools unify into one Tool.make whose run(input, emit) returns an Effect, and control/provider tools get honest kinds. Plus an additive Mistral provider.
---
