/**
 * Compare the recursive and pull-based `loop` implementations across iteration counts.
 * Run: `pnpm tsx experiments/streamLoopBench.ts`
 */
import { Effect, Stream } from "effect";
import * as StreamImpl from "./streamLoop.js";
import * as PullImpl from "./streamLoopPull.js";
...
const measure = async (
  ...
  const stream = impl.loop(0, (n) =>
    n >= N
      ? (Stream.fromIterable([n, impl.stop]) as Stream.Stream<
          number,
          never,
          never
        >)
      : (Stream.fromIterable([n, impl.next(n + 1)]) as Stream.Stream<
          number,
          never,
          never
        >),
...
const pullImpl = PullImpl as unknown as Impl;

const main = async () => {
  // Warm-up
  await measure("Stream", streamImpl, 100);
  await measure("Pull", pullImpl, 100);
  console.log("---");

  for (const N of [1_000, 5_000, 10_000]) {
    await measure("Stream", streamImpl, N);
    await measure("Pull", pullImpl, N);
    console.log();
  }

  // If Pull is linear, this should run quickly. If quadratic, slow.
  for (const N of [50_000, 100_000]) {
    console.log(`Pull only at N=${N}:`);
    await measure("Pull", pullImpl, N);
    console.log();
  }
};
