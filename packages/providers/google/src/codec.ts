...
// ---------------------------------------------------------------------------
// HTTP failure mapping, shared by every Google surface
// ---------------------------------------------------------------------------

/** `provider` names the surface that failed, so errors read `gemini` or `lyria`. */
export const transportFailure =
  (provider: string) =>
  (cause: unknown): AiError.AiError =>
    new AiError.Unavailable({ provider, raw: cause })

/** 413 is the inline-payload cap, which is a context limit on every surface. */
export const httpStatusError =
  (provider: string) =>
  (status: number, body: string): AiError.AiError =>
    Match.value(status).pipe(
      ...
      Match.when(
        403,
        (): AiError.AiError =>
          new AiError.AuthFailed({ provider, subtype: "permission", raw: body }),
      ),
      Match.when(
        402,
