...
/**
 * A directory a recipe reads back on the next run. Deliberately not
 * timestamped: a cache under `runDir` would miss every time. `--cache`
 * points somewhere else, `--no-cache` is up to the recipe.
 */
export const cacheDir = (recipe: string, argv: ReadonlyArray<string>): string =>
  Option.getOrElse(flagValue("cache", argv), () => `output/${recipe}/cache`)
