...
2. **Fallback ladders need provenance.** `withFallback({ retryableTags:
   ...
   providers; `ContentFiltered` is *terminal everywhere* and must not cascade.
...
- **`ContextLengthExceeded` separate from `InvalidRequest`.** This one is
  *recoverable by changing strategy* (compact, summarize, switch to a 200k-
...
- **`Cancelled` separate from everything.** This is the user-pressed-stop case;
  it's terminal but not a *failure* of the system. The audit (§"Cancellation /
...
```ts
type AiError =
  | { _tag: "RateLimited"; retryAfter?: Duration; scope?: "rpm"|"tpm"|"rpd"|"tpd"; provider: string; raw: unknown; requestId?: string }
  | { _tag: "Unavailable"; retryAfter?: Duration; status?: number; provider: string; raw: unknown; requestId?: string }
  ...
  | { _tag: "ContextLengthExceeded"; modelLimit?: number; requested?: number; provider: string; raw: unknown }
  | { _tag: "InvalidRequest"; param?: string; provider: string; raw: unknown; requestId?: string }
  | { _tag: "AuthFailed"; subtype: "auth"|"permission"|"billing"|"quota"; provider: string; raw: unknown }
...
*actually* came back from the provider?" debug session.
...
- **`content_filter` is a `finish_reason`, NOT an HTTP error.** This is the
  ...
  *after* normal stream consumption — not via the error channel. (See
...
- **`insufficient_quota` is a 429**, not a 402. This makes header-driven retry
  *wrong*: the SDK gets `Retry-After`, retries, gets the same 429 forever.
...
- **Safety blocks are NOT errors.** They come back via:
  ...
  into `ContentFiltered`. `MAX_TOKENS` is *not* an error — it's a normal
  truncation. `RECITATION` *is* a content filter (copyright). Sources:
...
- **`streamGenerateContent` SSE has no `error` event type.** A first-chunk
  error returns 400 *with an SSE-formatted body* (yes, really —
...
- **Rate-limit headers are sparsely documented.** The troubleshooting page
  ...
  *not always*. Adapter should default to exponential backoff w/ jitter when
...
- **Passes through upstream errors with metadata.** The unified shape is:
  ```json
  {
    "error": {
      "code": 402,
      "message": "...",
      "metadata": { "provider_name": "anthropic", "raw": { /* original */ } }
    }
  }
  ```
  Adapter should keep `metadata.raw` in `AiError.raw` and surface
  `metadata.provider_name` so downstream telemetry knows *which* upstream
  failed.
...
- **503 = "no available model provider that meets routing requirements."**
  This is `Unavailable`, but it's *terminal-for-the-current-config* — retry
...
- **Mid-stream errors are first-class events in the EventStream protocol.**
  `ConverseStreamOutput` is a tagged union where one variant *is* the
...
mitigation for this. Once the adapter has emitted *any* `TextDelta` downstream,
...
- `Retry-After` *sometimes* on 429s, but [the cloud blog explicitly says
