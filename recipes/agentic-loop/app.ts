/**
 * Composition for the agentic-loop recipe: the two tools, the stdin reader,
 * the renderer, and the `main` that ties them together. Reads lines from
 * stdin, sends them to the conversation queue, prints model output as it
 * streams. Ctrl-C exits.
 *
...
import { DateTime, Effect, Match, Option, Queue, Ref, Schema, Stdio, Stream } from "effect"
...
export const main = Effect.gen(function* () {
  ...
  yield* renderConversation(queue, streaming).pipe(Effect.provide(languageModelLayer(spec)))
})
