 ...
 * synthetic stream.
 *
 * Pre-packaged. `Retry.stream(schedule)` ships exactly this lift /
 * `Stream.retry` / unlift dance scoped to the `Retryable` subset of
 * `AiError`. There's an Effect-flavored sibling `Retry.effect` for
 * `turn` / `embed` / `synthesize` / `transcribe`. Reach for them when
 * you don't need to customize the lifted-item shape; this recipe spells
 * the pattern out for explainability.
 *
 * `recipe.ts` exports the `conversation`; composition lives in `app.ts`.
 */
...
export const conversation = (model: string) =>
  pipe(
    initial,
    loop((state) =>
      Effect.gen(function* () {
        const lm = yield* LanguageModel
        return lm.streamTurn({ history: state.history, model }).pipe(
