/**
 * Mic capture → PCM s16le @ 16 kHz mono, posted as ~50 ms frames over
 ...
 * `processorOptions` and resample to `targetRate` here with a simple
 * averaging decimator — good enough for STT.
 */
class MicWorklet extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.outOffset = 0
    // Fractional accumulator so non-integer ratios (e.g. 44.1k → 16k) don't drift.
    this.acc = 0
  }
