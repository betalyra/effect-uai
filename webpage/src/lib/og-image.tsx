...
async function loadFonts(): Promise<FontEntry[]> {
  ...
  const sans400 = await readFile(
    resolve(cwd, "node_modules/@fontsource/albert-sans/files/albert-sans-latin-400-normal.woff2"),
  )
  const sans800 = await readFile(
    resolve(cwd, "node_modules/@fontsource/albert-sans/files/albert-sans-latin-800-normal.woff2"),
  )
  const sans900 = await readFile(
    resolve(cwd, "node_modules/@fontsource/albert-sans/files/albert-sans-latin-900-normal.woff2"),
  )
...
async function loadLogoSvg(): Promise<Buffer> {
  ...
  cachedSvg = await readFile(resolve(process.cwd(), "src/assets/effect-uai-logo.svg"))
  return cachedSvg
}
...
async function rasterizeLogo(width: number, height: number): Promise<ArrayBuffer> {
...
export async function createOgImage({ title, subtitle, eyebrow }: OgImageInput): Promise<Response> {
  const fonts = await loadFonts()

  const isHomepage = title === "effect-uai"

  const titleSize = title.length > 80 ? 80 : title.length > 50 ? 104 : title.length > 30 ? 128 : 160
